...
1. Hardens an ultra-minimal Alpine Linux host (`192.168.10.2`) with an enforced OS gatekeeper.
2. Automates the deployment of PostgreSQL 15 and Mattermost Team Edition via [scripts/setup.sh](https://github.com/rabakuku/Youtube/blob/main/Send-Real-Time-Attack-Alerts-to-Mattermost-in-Docker/scripts/setup.sh).
...
All host configuration, package installations, repository enablement, and container orchestration have been automated into a single deployment script. Manual configuration of Docker or Compose is not required.

1. Log into your Alpine Linux host (`192.168.10.2`) as `root`.
2. Clone the repository and navigate to the project directory:
```bash
git clone [https://github.com/rabakuku/Youtube.git](https://github.com/rabakuku/Youtube.git) /opt/youtube
cd /opt/youtube/Send-Real-Time-Attack-Alerts-to-Mattermost-in-Docker

```


3. Make the automation scripts executable:
```bash
...
4. Execute the automated setup harness:
```bash
./scripts/setup.sh
...
* Installs `docker`, `docker-cli-compose`, `containerd`, `iptables`, `curl`, and `net-tools`.
* Registers the Docker service with the `boot` runlevel and starts the daemon via OpenRC.
* Pulls [compose/docker-compose.yml](https://github.com/rabakuku/Youtube/blob/main/Send-Real-Time-Attack-Alerts-to-Mattermost-in-Docker/compose/docker-compose.yml) and generates [compose/.env](https://github.com/rabakuku/Youtube/blob/main/Send-Real-Time-Attack-Alerts-to-Mattermost-in-Docker/compose/.env.example).
...
### Step 3: Configure FortiOS Automation Stitch & Address Objects
...
docker compose -f /opt/youtube/Send-Real-Time-Attack-Alerts-to-Mattermost-in-Docker/compose/docker-compose.yml ps
...
The script presents a menu allowing you to choose the exact level of removal:
...
Executes Option 1 and Option 2, purges all installed network and storage dependencies (`e2fsprogs`, `iptables`, `curl`, `net-tools`), removes `/var/lib/docker`, and wipes all generated configuration files to return the system to its initial baseline.
